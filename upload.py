...
def success():  
    if request.method == 'POST':  
        ...
        file_path=os.path.join(basepath,'Photo',secure_filename(f.filename))
        f.save(file_path)
        ...
        if len(result[0]) > 1: # check for multi-class
            pred_class = class_names[result.argmax()] # if more than one output, take the max
        else:
            pred_class = class_names[int(tf.round(result)[0][0])]
        os.remove(file_path)
        google_result = wikipedia.page(wikipedia.search(pred_class)[0])
        return render_template('success.html', name = pred_class, information = google_result.summary)
